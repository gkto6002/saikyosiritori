...
その後のエージェント改善で、Minimax、AlphaBeta、MonteCarlo、Greedy、Randomへ辺ネイティブ経路を追加し、BeamNegamaxとAggressivePVSを実装した。攻撃・生存評価、例外ベースのタイムアウト、共通適応深度も追加し、旧word方式は互換用に残した。完全解析も辺使用回数だけのメモ化再帰へ移行した。
...
- 単一・複数サイズ、単一・複数最大文字数、単一・複数seed CLI
...
- 理由付き人間入力検証と整合性検査
- 七つの近似AIを直接呼ぶ辺ネイティブ`simulate_runtime_match`
...
七つの近似AIを辺ネイティブな対局経路で利用できる。探索系4AIは共通の攻撃・生存評価、適応深度、タイムアウト例外を使う。完全解析もword maskから混合基数の辺使用回数コードへ移行し、勝ち初手辺を1本発見した時点でそのDの探索を終了する。
